...
DB에 저장된 내용을 이용하여 ATM기 만들기 및 잔액 변경 시 update
...
class ATM:
    ...
    '''def validate_password(self, account_num, password):        # 계좌정보 DB에서 입력받은 계좌번호 유무 확인 및 비밀번호 확인
        sql = "SELECT password FROM `user info 2` WHERE account=%s"
    ...
    def transaction(self, account_num):                             
        sql = "SELECT * FROM `user info 2` WHERE account=%s"        # 사용자 계좌 번호 확인
        self.cursor.execute(sql, (account_num,))
        ...
        while True:
            ...
            if transaction_type == "1":  # 입금
                ...
                # 입금 완료 시, DB에 잔액 업데이트
                update_sql = "UPDATE `user info 2` SET `initial balance`=%s WHERE account=%s"
                self.cursor.execute(update_sql, (float(balance), account_num))
                self.conn.commit()
                ...
                sql = "SELECT * FROM `user info` WHERE account=%s"
                self.cursor.execute(sql, (target_account_num,))
                target_account_data = self.cursor.fetchone()
                if target_account_data is None:
                    print("이체 대상 계좌번호가 유효하지 않습니다.")
                    return
                ...
                target_balance = float(target_account_data[3])
                target_balance += transfer_amount


                 # DB에 잔액 업데이트 (이체 대상 계좌)
                update_sql = "UPDATE `user info 2` SET `initial balance`=%s WHERE account=%s"
                self.cursor.execute(update_sql, (float(target_balance), target_account_num))
                self.conn.commit()
